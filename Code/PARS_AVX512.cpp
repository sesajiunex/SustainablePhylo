...
/**
*       Method that runs the OpenMP+AVX512 parsimony code for CPU. Parameters:
...
int PARS::doOpenMPPARSModCPU (int _popsize, int _num_threads)
{
        ...
        char line_fitch_sequences_omp[length_lfso][v] __attribute__((aligned (64)));
        ...
        for (i=0; i<_popsize; i++)
        {
                ...
                char my_characters[num_internal_nodes][v] __attribute__((aligned (64)));
                int indice;
                double accPars=0;
                ...
                __m512i and_site_son; //auxiliar variable for fitch operations
                __m512i or_site_son; //auxiliar variable for fitch operations
                __m512i site_value; //variable to store the state calculated for a node
                __m512i son_value; //variable to store the state read from a child node
                __m512i if_zero; //variable to store the if condition
                __m512i if_and;
                __m512i if_or;
                __m512i aux_pars; //variable to store the pars increment
                __m512i zeros = _mm512_setzero_si512(); //variable 0s
                __m512i ones = _mm512_set1_epi8(0x01); //variable 1s
                __mmask64 mask_cmp;
                ...
                for (j = 0; j < n_sites_vectorizado; j++)
                {
                                ...
                                {
                                        site_value = _mm512_set1_epi8(0xFF);
                                        ...
                                        for (l=0; l<num_sons; l++)
                                        {
                                                ...
                                                if (node_class == 0x80000000){
                                                        //LOAD
                                                        son_value = _mm512_load_si512(my_characters[node_id]);
                                                }
                                                else{
                                                        //LOAD
                                                        son_value = _mm512_load_si512(line_fitch_sequences_omp[indice]);
                                                }

                                                //Compute and / or between site_value and son _value
                                                and_site_son = _mm512_and_si512(site_value, son_value);
                                                or_site_son = _mm512_or_si512(site_value,son_value);

                                                //Compute masks for if ((site_value & son_value) == 0) cases
                                                mask_cmp = _mm512_cmpeq_epi8_mask(and_site_son,zeros);
                                                if_zero = _mm512_movm_epi8 (mask_cmp);

                                                //Identify cases that imply P(T)+++
                                                aux_pars = _mm512_and_si512(if_zero,ones);  
                                                
                                                //Adding cases
                                                aux_pars =_mm512_sad_epu8(zeros,aux_pars);

                                                //AVX512 Reduction
                                                partPars = _mm512_reduce_add_epi32 (aux_pars);
                                                Parsimony+=partPars;

                                                //Computing locations of and / or states
                                                if_and = and_site_son;
                                                if_or = _mm512_and_si512(if_zero,or_site_son);

                                                //Combine and / or results
                                                site_value=_mm512_or_si512(if_or,if_and);
                                        }

                                        //STORE
                                        _mm512_store_si512(my_characters[k],site_value);
                                }
                }
